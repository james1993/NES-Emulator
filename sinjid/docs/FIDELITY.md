...
**Item prices are the original's, and selling works by type.** Joining each
buy button to the screen it sits on recovers 48 prices, from a 10-gold Bandana
to 1600-gold Golden Armour. The remaining 21 rows are never stocked -- starting
weapons, enemy-only drops, and the food and drink written for this remake -- so
their prices are ours.

Selling is *not* a fraction of the asking price. The original matches an item's
type against a list and pays a flat rate, and only two such lists exist in the
whole file: Head Gear, Shield, Suit and Weapon pay **0**, and Drink pays **3**.
So equipment cannot be sold on at all, and a merchant refuses it.

**The herb trade is that flat rate put to work.** Medicine and White Leaves
both carry zero stats -- they are trade goods, not consumables -- and the leaves
cost nothing on the screen that stocks them. The herbalist's script walks the
whole pack and converts every White Leaves in one visit, so the loop is: take
leaves, have them worked into medicine, sell each for 3. Small, and repeatable,
which is what it is in the original.
